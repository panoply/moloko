...
// src/model.ts
var _a;
var model = new (_a = class {
  constructor() {
    this.config = _a.config;
    this.hash = "";
    this.open = null;
    this.path = urlJoin(_a.config.resolve.origin, _a.config.resolve.path);
    this.documents = {
    ...
    this.rules = {
      ...
      model: null,
      state: 1 /* Hidden */,
      esthetic: _a.config.rules.esthetic,
      node: null
    };
  }
  update(options) {
    this.config = mergerino_min_default(this.config, options);
    this.hash = this.config.hash ? "" : null;
    this.path = urlJoin(this.config.resolve.origin, this.config.resolve.path);
...
var esthetic;
var m;
async function load(attrs) {
  if (attrs.config.resolve.esthetic === true) {
    try {
      const esm = await import(urlJoin(attrs.path, "modules/esthetic.js"));
  ...
  if (attrs.config.resolve.mithril === true) {
    try {
      await import(urlJoin(attrs.path, "modules/mithril.js"));
      m = window.m;
    } catch (e2) {
  ...
  if (typeof attrs.config.samples === "boolean") {
    if (attrs.config.samples === true) {
      for (const language in attrs.documents) {
        try {
          const code = await import(urlJoin(attrs.path, `sample/${language}.js`));
          attrs.documents[language].sample = code.default;
        } catch (e2) {
          throw new Error(`Failed to import ${language} code sample`, e2);
        }
      }
    }
    ...
    for (const language in attrs.config.samples) {
      if (attrs.config.samples[language] === true) {
        try {
          console.log(urlJoin(attrs.path, `sample/${language}.js`));
          const code = await import(urlJoin(attrs.path, `sample/${language}.js`));
          attrs.documents[language].sample = code.default;
        } catch (e2) {
          throw new Error(`Failed to import ${language} code sample`, e2);
...
// src/editor/sidebar.ts
function isActive(attrs, key) {
  if (attrs.table.state === 3 /* Opened */ && (key === "preview" || key === "rules"))
    return "disable";
  if (attrs.rules.state === 3 /* Opened */) {
    if (key === "rules")
      return "active";
    if (key === "table" || key === "preview")
      return "disable";
  }
  if (key === "file" && attrs.language.state === 3 /* Opened */)
    return "active";
  if (key === "table" && attrs.table.state === 3 /* Opened */)
    return "active";
  return "";
}
function getTooltip(attrs, key, action) {
  if (key === "preview") {
    if (isOpen(attrs.table.state))
      return "Disabled in Parse Table Mode";
    if (isOpen(attrs.rules.state))
      return "Previewing Rule Changes";
  } else if (key === "rules") {
    if (isOpen(attrs.table.state))
      return "Disabled in Parse Table Mode";
  } else if (key === "table") {
    if (isOpen(attrs.rules.state))
      return "Disabled in Rule Mode";
  }
  return action.tooltip;
}
function getIcon(attrs, key, action) {
  if (key === "preview") {
    if (attrs.rules.state === 3 /* Opened */)
      return icon("eye");
    return attrs.preview.state === 3 /* Opened */ ? icon(action.icon) : icon("document");
  }
  return icon(action.icon);
}
function onPreview(attrs, callback = null) {
  if (attrs.preview.state === 2 /* Toggle */ || attrs.preview.state === 4 /* Reopen */ || attrs.rules.state === 3 /* Opened */)
    return;
  if (attrs.preview.state === 3 /* Opened */) {
    if (attrs.table.state === 3 /* Opened */) {
      attrs.table.state = 2 /* Toggle */;
      attrs.table.node.style.flexGrow = "0";
      attrs.table.node.ontransitionend = () => {
        attrs.table.state = 1 /* Hidden */;
        attrs.rules.state = 1 /* Hidden */;
        callback && callback();
        m.redraw();
      };
    }
    attrs.preview.state = 2 /* Toggle */;
    attrs.preview.node.style.flexGrow = "0";
    attrs.preview.node.ontransitionend = () => {
      attrs.preview.state = 1 /* Hidden */;
      callback && callback();
      m.redraw();
    };
  } else if (attrs.preview.state === 1 /* Hidden */) {
    attrs.preview.state = 2 /* Toggle */;
    attrs.preview.node.style.flexGrow = "1";
    attrs.preview.node.ontransitionend = () => {
      attrs.preview.state = 3 /* Opened */;
      callback && callback();
      m.redraw();
    };
  }
}
function onRules(attrs, callback = null) {
  if (attrs.rules.state === 2 /* Toggle */ || attrs.rules.state === 4 /* Reopen */)
    return;
  if (attrs.rules.state === 3 /* Opened */) {
    if (attrs.input.state === 4 /* Reopen */) {
    ...
    attrs.rules.state = 2 /* Toggle */;
    attrs.rules.node.style.flexGrow = "0";
    attrs.rules.node.ontransitionend = () => {
      attrs.rules.state = 1 /* Hidden */;
      attrs.table.state = 1 /* Hidden */;
      attrs.input.state = 3 /* Opened */;
      formatCode(attrs);
      callback && callback();
      m.redraw();
    };
  } else if (attrs.rules.state === 1 /* Hidden */) {
    if (attrs.language.state === 3 /* Opened */) {
      onLanguage(attrs, () => onRules(attrs));
    } else {
      attrs.rules.node.style.flexGrow = ".6";
      attrs.rules.state = 2 /* Toggle */;
      attrs.table.state = 5 /* Disable */;
      attrs.preview.node.style.flexGrow = "1";
      attrs.input.node.style.flexGrow = "0";
      if (attrs.preview.state === 1 /* Hidden */) {
        attrs.preview.state = 2 /* Toggle */;
        attrs.input.state = 4 /* Reopen */;
      }
      attrs.rules.node.ontransitionend = () => {
        attrs.rules.state = 3 /* Opened */;
        attrs.preview.state = 3 /* Opened */;
        callback && callback();
        m.redraw();
      };
...
function onLanguage(attrs, callback = null) {
  if (attrs.language.state === 3 /* Opened */) {
...
function onParseTable(attrs) {
  if (attrs.table.state === 2 /* Toggle */ || attrs.table.state === 4 /* Reopen */)
    return;
  if (attrs.table.state === 3 /* Opened */) {
    if (attrs.preview.state === 4 /* Reopen */) {
      attrs.preview.state = 2 /* Toggle */;
      attrs.preview.node.style.flexGrow = "1";
      attrs.preview.node.ontransitionend = () => {
        attrs.preview.state = 3 /* Opened */;
  ...
  } else if (attrs.table.state === 1 /* Hidden */) {
    if (attrs.preview.state === 3 /* Opened */) {
      attrs.preview.state = 2 /* Toggle */;
      attrs.preview.node.style.flexGrow = "0";
      attrs.preview.node.ontransitionend = () => {
        attrs.preview.state = 4 /* Reopen */;
        m.redraw();
      };
    }
    attrs.table.state = 2 /* Toggle */;
    ...
    attrs.table.node.ontransitionend = () => {
      attrs.table.state = 3 /* Opened */;
      attrs.rules.state = 5 /* Disable */;
      m.redraw();
    };
  }
}
function onCopy(action, attrs, state) {
...
var PotionThemeLightBackground = {
  base: "vs-dark",
  inherit: true,
  colors: {
...
// src/monaco.ts
var monaco;
async function getMonacoModule(path) {
  loadExternalCSS(path);
  monaco = await import(urlJoin(path, "monaco/monaco.js"));
...
var Preview = {
  ...
  view: ({
    ...
    "moloko-preview",
    {
      ariaLabel: attrs.preview.state === 3 /* Opened */ ? "Output Preview" : "",
      style: {
...
// src/editor/rules.ts
function RulesStatic(dom, attrs) {
  const options = assign(
    {},
    attrs.config.monaco
  );
  ...
  return {
    mount: () => {
      attrs.rules.editor = monaco.editor.create(dom, options);
        ...
        ([
          ...
          "button.language",
          {
            ...
            onclick: () => {
              ...
              const value = attrs.documents[language].input === "" ? attrs.documents[language].sample : attrs.documents[language].input;
...
function onClickHighlight(attrs, i) {
  active.clear();
...
async function loader(options = {}) {
  ...
  try {
    ...
    throw new Error("Failed to load Monaco Editor Module", e2);
  }
